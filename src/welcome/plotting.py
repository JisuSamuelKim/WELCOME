...
def plot_power_realisations(result, use_cache: bool = True):
    """Compare every theoretical C_ell realisation with its emulated-map spectrum."""
    ...
    common_max = min(int(theory_ells[-1]), int(emu_ells[-1]))
    common = np.arange(2, common_max + 1, dtype=np.int64)
    theory_pos = np.searchsorted(theory_ells, common)
    emu_pos = common
    theory_match = theory_cls[:, theory_pos]
    emu_match = emu_cls[:, emu_pos]

    n = min(theory_match.shape[0], emu_match.shape[0])
    theory_match = theory_match[:n]
    emu_match = emu_match[:n]
    ...
    for index in range(n):
        ax.loglog(common, theory_match[index], linewidth=0.7, alpha=0.10, color=theory_colour)
        ax.loglog(common, emu_match[index], linewidth=0.7, alpha=0.10, color=emu_colour)
        residual = 100.0 * (emu_match[index] / theory_match[index] - 1.0)
        ax_res.semilogx(common, residual, linewidth=0.6, alpha=0.08, color=emu_colour)

    theory_mean = np.mean(theory_match, axis=0)
    emu_mean = np.mean(emu_match, axis=0)
    ax.loglog(common, theory_mean, linewidth=2.2, color=theory_colour, label="Theory mean")
    ax.loglog(common, emu_mean, linewidth=2.2, linestyle="--", color=emu_colour, label="Emulation mean")

    mean_residual = 100.0 * (emu_mean / theory_mean - 1.0)
    ax_res.semilogx(common, mean_residual, linewidth=2.0, color=emu_colour, label="Mean residual")
    ax_res.axhline(0.0, linewidth=1.0)

    ax.set_ylabel(r"$C_\ell$")
    ax.set_title(f"Matched theory and emulation power-spectrum realisations (N={n})")
    ax.legend(frameon=False)
    ax_res.set_xlabel(r"Multipole $\ell$")
    ax_res.set_ylabel("Emu/Theory - 1 [%]")
    fig.tight_layout()
...
def measure_emulated_wavelet_l1(
    ...
    # Pass 1 establishes a common histogram support across all realisations.
    for index, path in enumerate(paths):
        field = np.load(path, mmap_mode="r", allow_pickle=False)
...
def prepare_wavelet_diagnostics(
    ...
    """Compute/load theory curves and matched emulation wavelet L1 measurements."""
    bands = tuple((float(a), float(b)) for a, b in bands)
    theory = compute_theoretical_wavelet_l1_scales(result["config"], bands=bands, save=True)
    ...
    return {"bands": bands, "theory": theory, "emulation": emulation}


def plot_wavelet_l1(diagnostics):
    """Plot deterministic theoretical L1 curves against the emulation ensemble."""
    ...
    for scale_index, ((theta1, theta2), ax) in enumerate(zip(bands, axes)):
        label = f"{theta1:g}-{theta2:g}"
        theory = theory_products[label]
        emu_mean = np.nanmean(all_l1[:, scale_index], axis=0)
        emu_low = np.nanpercentile(all_l1[:, scale_index], 16.0, axis=0)
        emu_high = np.nanpercentile(all_l1[:, scale_index], 84.0, axis=0)

        ax.plot(theory["kappa"], theory["l1"], linewidth=2.0, label="Theory")
        ax.plot(kappa[scale_index], emu_mean, linewidth=2.0, linestyle="--", label="Emulation mean")
        ax.fill_between(kappa[scale_index], emu_low, emu_high, alpha=0.18, label="Emulation 16-84%")
        ax.set_title(rf"${theta1:g}'-{theta2:g}'$")
        ax.set_xlabel(r"$\kappa$")
        ax.set_ylabel(r"$|\kappa|P(\kappa)$")
        ax.legend(frameon=False, fontsize=9)

    fig.suptitle("Theory and emulation wavelet L1-norm")
    fig.tight_layout()
    return fig, axes
