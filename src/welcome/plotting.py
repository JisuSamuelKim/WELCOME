...
def plot_power_realisations(result, use_cache: bool = True):
    ...
    if not np.any(keep):
        raise ValueError("No theory power-spectrum bins are fully contained in the emulation ell range.")

    B = binning_matrix[keep]
    ell_eff = bin_ell_eff[keep]

    # The theory spectra already live on the same ell grid as the binning matrix.
    theory_binned = theory_cls @ B.T

    # Place each measured emulation spectrum on the theory ell grid, then apply
    # exactly the same binning matrix. Retained bins are fully inside the
    # available emulation multipole range.
