...
def plot_wavelet_l1(
    diagnostics,
    residual_min_peak_fraction: float = 0.01,
    support_peak_fraction: float = 0.01,
):
    """Plot theory/emulation L1 curves and theory-normalised residuals."""
    ...
    for scale_index, (theta1, theta2) in enumerate(bands):
        ...
        lo, hi = _information_support(
            theory["kappa"],
            theory["l1"],
            peak_fraction=support_peak_fraction,
        )
        support_minima.append(lo)
        support_maxima.append(hi)
    ...
    common_min = max(support_minima)
    common_max = min(support_maxima)
    if (
    ...
    theory_sigma = np.empty(len(bands), dtype=np.float64)
    ...
    for scale_index, (theta1, theta2) in enumerate(bands):
        ...
        theory_sigma[scale_index] = _theory_pdf_sigma(theory)
    ...
    fig, (ax, ax_res) = plt.subplots(
        ...
        figsize=(11.0, 8.5),
        sharex=False,
        gridspec_kw={"height_ratios": [3.0, 1.25], "hspace": 0.10},
    )
    ...
    for scale_index, (theta1, theta2) in enumerate(bands):
        ...
        x_residual = common_kappa / theory_sigma[scale_index]
        visible = valid_residual & (x_residual >= -2.0) & (x_residual <= 2.0)
        ax_res.plot(
            x_residual[visible],
            residual[visible],
            linestyle="--",
        ...
        scale_handles.append(
            Line2D([0], [0], color=colour, label=scale_label)
        )

    style_handles = [
        Line2D([0], [0], color="black", linestyle="-", label="Theory"),
        Line2D([0], [0], color="black", linestyle="--", label="Emulation"),
    ]
    ...
    ax_res.axhline(0.0, color="black", zorder=1)

    ax.set_ylabel(r"$\ell_1(\kappa)$")
    ax_res.set_xlabel(r"$\kappa/\sigma_{\rm theory}$")
    ax_res.set_ylabel("Residual [%]")
    ...
    ax_res.set_yticks([-10.0, -5.0, 0.0, 5.0, 10.0])
    ax_res.margins(x=0.0)
