...
def _serialisable_config(config: WelcomeConfig) -> dict:
    """Return a JSON-serialisable representation of a WELCOME configuration."""
    data = asdict(config)
    for key, value in tuple(data.items()):
        if isinstance(value, Path):
            data[key] = str(value)
    if data.get("nz_file") is not None:
        data["nz_file"] = str(data["nz_file"])
    data["output_dir"] = str(data["output_dir"])
    return data


def _write_run_manifest(config: WelcomeConfig) -> Path:
    """Save lightweight run metadata for future compatibility checks."""
    path = Path(config.output_dir) / "welcome_run_config.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "format_version": 1,
...
def _manifest_mismatches(config: WelcomeConfig, manifest_path: Path) -> list[str]:
    """Compare result-defining configuration fields with a saved manifest."""
    try:
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        saved = payload.get("config", {})
    except Exception as exc:
        return [f"could not read {manifest_path.name}: {exc}"]

    current = _serialisable_config(config)
    mismatches = []
    for key in _RESULT_COMPATIBILITY_FIELDS:
        if key not in saved:
            continue
        a = saved[key]
        b = current[key]
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            if not np.isclose(float(a), float(b), rtol=0.0, atol=1.0e-12):
...
def _validate_loaded_power_product(power_product: dict, config: WelcomeConfig) -> list[str]:
    """Return practical compatibility problems for an existing theory product."""
    problems = []

    checks = {
        "nside": int(config.theory_nside),
        "ell_min": int(config.theory_ell_min),
        "ell_max": int(config.theory_ell_max),
        "n_bins": int(config.n_power_spectrum_bins),
        "sampling_seed": int(config.theory_random_seed),
    }
    for key, expected in checks.items():
        if key in power_product and int(np.asarray(power_product[key]).item()) != expected:
            problems.append(f"{key} differs")

    if "n_realisations" in power_product:
        available = int(np.asarray(power_product["n_realisations"]).item())
        if available < int(config.n_realisations):
            problems.append(
                f"only {available} theoretical realisations are stored, "
                f"but {config.n_realisations} are requested"
            )

    if "sigma8" in power_product:
        saved_sigma8 = float(np.asarray(power_product["sigma8"]).item())
        if not np.isclose(saved_sigma8, float(config.sigma8), rtol=0.0, atol=1.0e-10):
            problems.append("sigma8 differs")

    if "halofit_version" in power_product:
        saved_halofit = str(np.asarray(power_product["halofit_version"]).item())
        if saved_halofit != str(config.halofit_version):
            problems.append("halofit_version differs")

    if "source_redshift" in power_product:
        saved_source = float(np.asarray(power_product["source_redshift"]).item())
        if not np.isclose(
            saved_source,
            float(SOURCE_EFFECTIVE_REDSHIFT),
            rtol=0.0,
            atol=1.0e-8,
        ):
            problems.append("source redshift differs")
    ...
    missing = sorted(required - set(power_product))
    if missing:
        problems.append("missing power-product keys: " + ", ".join(missing))

    return problems


def _load_existing_emulation_results(config: WelcomeConfig) -> tuple[list[dict], list[str]]:
    """Load existing map/history records without regenerating maps."""
    results = []
    problems = []

    for realisation_index in range(int(config.n_realisations)):
        map_path = emulation_map_path(realisation_index)
        history_path = emulation_history_path(realisation_index)

        if not map_path.exists():
            problems.append(f"missing map: {map_path.name}")
            continue
        if not history_path.exists():
            problems.append(f"missing history: {history_path.name}")
            continue
        ...
        try:
            with np.load(history_path, allow_pickle=False) as h:
                ...
                missing = required - set(h.files)
                if missing:
                    problems.append(
                        f"{history_path.name} is missing metadata: "
                        + ", ".join(sorted(missing))
                    )
                    continue

                expected_seed = int(config.random_seed) + int(realisation_index)
                checks = (
                    (int(h["nside"]) == int(EMULATION_NSIDE), "nside"),
                    (int(h["lmax"]) == int(EMULATION_LMAX), "lmax"),
                    (int(h["n_iterations"]) == int(N_ITERATIONS), "n_iterations"),
                    (int(h["random_seed"]) == expected_seed, "random_seed"),
                    (int(h["harmonic_iter"]) == int(HARMONIC_ITER), "harmonic_iter"),
                    (str(h["source_tag"]) == source_token(), "source_tag"),
                )
                failed = [label for ok, label in checks if not ok]
                if failed:
                    problems.append(
                        f"{history_path.name} differs in: " + ", ".join(failed)
                    )
                    continue
                ...
                results.append(
                    {
                        ...
                        "mean_history": np.asarray(
                            h["mean_l1_error_percent"], dtype=np.float64
                        ),
                        "scale_history": np.asarray(
                            h["scale_l1_error_percent"], dtype=np.float64
                        ),
                        "skipped": True,
                    }
                )
        except Exception as exc:
            problems.append(f"could not load {history_path.name}: {exc}")

    return results, problems


def load_welcome_result(
    config: WelcomeConfig | None = None,
    *,
    run_if_missing: bool = True,
):
    """Load an existing WELCOME run from ``config.output_dir``.

    This is the practical restart-friendly loader used by the example notebook.
    Existing theoretical products and map histories are reused when they are
    compatible with the requested map setup. Legacy runs without a manifest are
    accepted using the metadata already stored in the NPZ products.

    If required products are genuinely missing and ``run_if_missing`` is True,
    the complete WELCOME pipeline is run. Existing but incompatible products are
    never silently overwritten; use a different ``output_dir`` for a new setup.
    """
    if config is None:
        config = WelcomeConfig()
    ...
    manifest_path = BASE_OUTPUT_DIR / "welcome_run_config.json"
    if manifest_path.exists():
        mismatches = _manifest_mismatches(config, manifest_path)
        if mismatches:
            raise RuntimeError(
                "Existing WELCOME output is incompatible with the current "
                "configuration in: "
                + ", ".join(mismatches)
                + ". Use a different OUTPUT_DIR for this configuration."
            )

    missing_theory = [
        path for path in (TARGET_POWER_NPZ, THEORY_ALL_SCALE_NPZ) if not path.exists()
    ]
    if missing_theory:
        if not run_if_missing:
            raise FileNotFoundError(
                "Missing WELCOME products: "
                + ", ".join(str(path) for path in missing_theory)
            )
        print("[WELCOME] Existing complete result not found. Running WELCOME.")
        return run_welcome(config)

    power_product = load_npz_product(TARGET_POWER_NPZ)
    power_problems = _validate_loaded_power_product(power_product, config)
    if power_problems:
        raise RuntimeError(
            "Existing theoretical power product is incompatible: "
            + "; ".join(power_problems)
            + ". Use a different OUTPUT_DIR for this configuration."
        )

    wavelet_product = load_npz_product(THEORY_ALL_SCALE_NPZ)

    emulation_results, map_problems = _load_existing_emulation_results(config)
    if map_problems:
        if not run_if_missing:
            raise FileNotFoundError(
                "Existing WELCOME map set is incomplete: " + "; ".join(map_problems)
            )
        print("[WELCOME] Theory products found, but the map set is incomplete.")
        print("[WELCOME] Generating only missing/incompatible map realisations.")
        emulation_results = generate_emulated_maps(config)

    if len(emulation_results) != int(config.n_realisations):
        raise RuntimeError(
            f"Expected {config.n_realisations} emulation results, "
            f"but loaded {len(emulation_results)}."
        )

    if not manifest_path.exists():
        _write_run_manifest(config)

    print(f"[WELCOME] Existing result loaded from: {BASE_OUTPUT_DIR}")
